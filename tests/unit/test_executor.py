...
from src.vpsweb.core.executor import StepExecutor, StepExecutorError, PromptRenderingError, LLMCallError, OutputParsingError
from src.vpsweb.models.config import StepConfig, ModelProviderConfig
from src.vpsweb.models.translation import TranslationInput, InitialTranslation, EditorReview, RevisedTranslation
...
踏着猫的小脚。</initial_translation>
    ...
    @pytest.mark.asyncio
    async def test_execute_step_success(self, step_executor, mock_llm_factory, mock_prompt_service, sample_step_config, sample_llm_response):
        ...
        # Execute step
        result = await step_executor.execute_step("initial_translation", input_data, sample_step_config)
        ...
        assert result["output"]["initial_translation_notes"] == "This translation captures the gentle imagery."
        ...
        mock_prompt_service.render_prompt.assert_called_once_with("initial_translation", input_data)
        mock_provider.generate.assert_called_once()

    @pytest.mark.asyncio
    async def test_execute_step_with_retry_success(self, step_executor, mock_llm_factory, mock_prompt_service, sample_step_config, sample_llm_response):
        ...
        input_data = {"original_poem": "Test poem", "source_lang": "English", "target_lang": "Chinese"}
    ...
    @pytest.mark.asyncio
    async def test_execute_step_retry_exhausted(self, step_executor, mock_llm_factory, mock_prompt_service, sample_step_config):
        ...
        with pytest.raises(LLMCallError) as exc_info:
            await step_executor.execute_step("initial_translation", input_data, sample_step_config)
    ...
    @pytest.mark.asyncio
    async def test_execute_step_prompt_rendering_error(self, step_executor, mock_prompt_service, sample_step_config):
        ...
        mock_prompt_service.render_prompt.side_effect = TemplateLoadError("Template not found")
    ...
    @pytest.mark.asyncio
    async def test_execute_step_output_parsing_error(self, step_executor, mock_llm_factory, mock_prompt_service, sample_step_config):
    ...
    @pytest.mark.asyncio
    async def test_execute_initial_translation(self, step_executor, mock_llm_factory, mock_prompt_service, sample_step_config, sample_llm_response, sample_translation_input):
        ...
        result = await step_executor.execute_initial_translation(sample_translation_input, sample_step_config)
    ...
    @pytest.mark.asyncio
    async def test_execute_editor_review(self, step_executor, mock_llm_factory, mock_prompt_service, sample_step_config):
        ...
        result = await step_executor.execute_editor_review(initial_translation, sample_step_config)
    ...
    @pytest.mark.asyncio
    async def test_execute_translator_revision(self, step_executor, mock_llm_factory, mock_prompt_service, sample_step_config):
        ...
        result = await step_executor.execute_translator_revision(editor_review, sample_step_config)
        ...
        assert call_args[1]["editor_suggestions"] == "Consider using more poetic language"
    ...
    def test_validate_step_inputs(self, step_executor):
        ...
        # Valid inputs
        step_executor._validate_step_inputs("initial_translation", {"key": "value"}, config)

        # Invalid step name
        with pytest.raises(ValueError, match="Unknown step name"):
            step_executor._validate_step_inputs("invalid_step", {"key": "value"}, config)

        # Invalid input data type
        with pytest.raises(ValueError, match="Input data must be a dictionary"):
            step_executor._validate_step_inputs("initial_translation", "invalid", config)

        # Missing config
        with pytest.raises(ValueError, match="Step configuration is required"):
            step_executor._validate_step_inputs("initial_translation", {"key": "value"}, None)
    ...
    def test_build_step_result(self, step_executor, sample_llm_response, sample_step_config):
        ...
        assert metadata["model_info"]["model"] == "gpt-3.5-turbo"
        assert metadata["usage"]["tokens_used"] == 150
        assert metadata["raw_response"]["content_length"] == len(sample_llm_response.content)
        assert "content_preview" in metadata["raw_response"]
        assert "timestamp" in metadata

    def test_step_executor_repr(self, step_executor, mock_llm_factory, mock_prompt_service):
    ...
    @pytest.mark.asyncio
    async def test_empty_llm_response_handling(self, step_executor, mock_llm_factory, mock_prompt_service, sample_step_config):
    ...
    @pytest.mark.asyncio
    async def test_plain_text_response_fallback(self, step_executor, mock_llm_factory, mock_prompt_service, sample_step_config):
        ...
        result = await step_executor.execute_step("initial_translation", input_data, config_no_required)
