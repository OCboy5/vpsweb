...
class TestPoemEndpoints:
    ...
    async def test_list_poems_pagination(
        self, test_client, test_context  # Remove type hint
    ):
        ...
        # Test first page
        response = await test_client.get("/api/v1/poems/?page=1&page_size=2")
        assert response.status_code == 200
        data = response.json()
        ...
        # Test second page
        response = await test_client.get("/api/v1/poems/?page=2&page_size=2")
        assert response.status_code == 200
        data = response.json()
        assert len(data["poems"]) == 2
        assert data["pagination"]["has_next"] is True
    ...
    async def test_list_poems_filters(
        self, test_client, test_context  # Remove type hint to allow AsyncClient
    ):
        ...
        response = await test_client.get("/api/v1/poems/?poet_name=李白")
        assert response.status_code == 200
        data = response.json()
        assert len(data["poems"]) == 1
        ...
        data = response.json()
        assert len(data["poems"]) == 1
        assert data["poems"][0]["source_language"] == "en"

        # Test title search
        response = await test_client.get("/api/v1/poems/?title_search=Sonnet")
        assert response.status_code == 200
        data = response.json()
        assert len(data["poems"]) == 1
    ...
    @pytest.mark.asyncio
    async def test_create_poem_success(self, test_client: AsyncClient):
...
And never stops at all,""",
    ...
    @pytest.mark.asyncio
    async def test_create_poem_validation_errors(self, test_client: AsyncClient):
    ...
    @pytest.mark.asyncio
    async def test_get_poem_success(self, test_client: AsyncClient, sample_poem):
    ...
    @pytest.mark.asyncio
    async def test_get_poem_not_found(self, test_client: AsyncClient):
    ...
    @pytest.mark.asyncio
    async def test_update_poem_success(self, test_client: AsyncClient, sample_poem):
    ...
    @pytest.mark.asyncio
    async def test_update_poem_not_found(self, test_client: AsyncClient):
    ...
    @pytest.mark.asyncio
    async def test_toggle_poem_selection(self, test_client: AsyncClient, sample_poem):
    ...
    @pytest.mark.asyncio
    async def test_delete_poem_success(self, test_client: AsyncClient, sample_poem):
        """Test DELETE /api/v1/poems/{poem_id} with existing poem."""
        response = await test_client.delete(f"/api/v1/poems/{sample_poem.id}")
        ...
        get_response = await test_client.get(f"/api/v1/poems/{sample_poem.id}")
        assert get_response.status_code == 404

    @pytest.mark.asyncio
    async def test_delete_poem_not_found(self, test_client: AsyncClient):
        """Test DELETE /api/v1/poems/{poem_id} with non-existent poem."""
        fake_id = str(uuid.uuid4())[:26]
        response = await test_client.delete(f"/api/v1/poems/{fake_id}")

        assert response.status_code == 404

    @pytest.mark.asyncio
    async def test_get_poem_translations_empty(
    ...
    @pytest.mark.asyncio
    async def test_get_poem_translations_with_data(
        self, test_client: AsyncClient, sample_poem, sample_translation
    ):
    ...
    def test_get_filter_options(self, test_client: TestClient, test_context):
        ...
        test_context.create_poem(poet_name="Du Fu", source_language="zh-CN")
        ...
        assert "zh-CN" in data["languages"]
...
class TestTranslationEndpoints:
    ...
    @pytest.mark.asyncio
    async def test_list_translations_empty(self, test_client: AsyncClient):
    ...
    @pytest.mark.asyncio
    async def test_list_translations_with_data(
        self, test_client: AsyncClient, sample_translation
    ):
    ...
    def test_trigger_translation_validation_errors(self, test_client: TestClient):
        ...
        invalid_request = {"target_lang": "zh-CN", "workflow_mode": "hybrid"}
    ...
    async def test_list_translations_filters(
        ...
        poem = test_context.create_poem(poet_name="Test Poet", poem_title="Test Poem")

        test_context.create_translation(
            poem_id=poem.id, target_language="zh-CN", translator_type="ai"
        )
        test_context.create_translation(
            poem_id=poem.id,
            target_language="ja",
        ...
        response = test_client.get("/api/v1/translations/?target_language=zh-CN")
        assert response.status_code == 200
        data = response.json()
        assert len(data["translations"]) == 1
        assert data["translations"][0]["target_language"] == "zh-CN"
...
class TestMultiEntityIntegration:
    ...
    async def test_poem_translation_workflow(
        ...
        japanese_translation = test_context.create_translation(
            poem_id=poem.id,
            target_language="ja",
        ...
        assert "zh-CN" in target_languages
        assert "ja" in target_languages
    ...
    async def test_filter_and_search_integration(
        ...
        for poem_data in poems_data:
            poem = test_context.create_poem(**poem_data)
            created_poems.append(poem)

        # Test combined filters
        response = await test_client.get("/api/v1/poems/?language=zh-CN")
        assert response.status_code == 200
        data = response.json()
        assert len(data["poems"]) == 2
