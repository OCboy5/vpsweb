...
class WeChatArticleMetadata(BaseModel):
    ...
    series_index: Optional[str] = Field(None, description="Series index (e.g., '其一', '其二')")
    ...
    # File paths
    source_json_path: str = Field(..., description="Path to original translation JSON file")
    ...
    # Timestamps
    created_at: datetime = Field(default_factory=datetime.now, description="Article creation timestamp")
    ...
    def validate_slug_format(cls, v):
        ...
        if not re.match(pattern, v):
            raise ValueError(f"Slug '{v}' does not match expected format 'poetname-poemtitle-YYYYMMDD'")
        return v
...
class WeChatArticle(BaseModel):
    ...
    thumb_media_id: Optional[str] = Field(default=None, description="Thumb media_id for cover image")
    ...
    need_open_comment: bool = Field(default=False, description="Whether to enable comments")
    only_fans_can_comment: bool = Field(default=False, description="Whether only fans can comment")
    content_source_url: str = Field(default="", description="Source URL for original article")
    ...
    source_lang: str = Field(..., description="Source language of poem")
    target_lang: str = Field(..., description="Target language of translation")
    translation_workflow_id: str = Field(..., description="Original translation workflow ID")
    translation_json_path: str = Field(..., description="Path to original translation JSON file")

    # Translation notes
    translation_notes: Optional[TranslationNotes] = Field(None, description="Translation notes generated by LLM")
    ...
    def validate_title_format(cls, v):
        ...
        if len(v) > 60:
            raise ValueError("Title exceeds 60 character limit (64 allowed by WeChat API)")
        return v.strip()
    ...
    def validate_digest_length(cls, v):
        ...
        if len(v) > 115:
            raise ValueError("Digest must be at most 115 characters (120 allowed by WeChat API)")
        return v
...
class WeChatConfig(BaseModel):
    ...
    base_url: str = Field(default="https://api.weixin.qq.com", description="WeChat API base URL")
...
class ArticleGenerationResult(BaseModel):
    ...
    llm_metrics: Optional[Dict[str, Any]] = Field(None, description="LLM call metrics (tokens, duration, cost)")
...
class PublishingResult(BaseModel):
    ...
    # API response
    api_response: Optional[WeChatApiResponse] = Field(None, description="WeChat API response")
    ...
    created_at: datetime = Field(default_factory=datetime.now, description="Result creation timestamp")
...
class WeChatLLMModelsConfig(BaseModel):
    ...
    non_reasoning: WeChatLLMConfig = Field(..., description="Non-reasoning model configuration")
...
class ArticleGenerationConfig(BaseModel):
    ...
    article_template: str = Field(default="default", description="Article template to use")
