...
def demo_repository_service():
    ...
    print(
        f"✓ Created translations: AI ({trans1.translator_type}), Human ({trans2.translator_type})"
    )
    ...
    for poem in dashboard["recent_poems"]:
        ...
        for trans in translations:
            print(
                f"  🌐 {trans.translator_type.title()} translation to {trans.target_language}"
            )
            ...
            # Show preview
            preview = (
                trans.translated_text[:100] + "..."
                if len(trans.translated_text) > 100
                else trans.translated_text
            )
            ...
            if details["ai_logs"]:
                for ai_log in details["ai_logs"]:
                    print(
                        f"     🤖 AI: {ai_log.model_name} ({ai_log.runtime_seconds}s)"
                    )
            if details["human_notes"]:
                for note in details["human_notes"]:
                    note_preview = (
                        note.note_text[:80] + "..."
                        if len(note.note_text) > 80
                        else note.note_text
                    )
    ...
    print(
        f"  Page {paginated['pagination']['current_page']} of {paginated['pagination']['total_pages']}"
    )
    print(
        f"  Showing {len(paginated['poems'])} of {paginated['pagination']['total_items']} poems"
    )
