...
def upgrade() -> None:
    ...
    # WorkflowTask tracking now handled by FastAPI app.state for in-memory storage

    # Composite indexes for HumanNote table
    # Common queries: "Get notes by translation with pagination"
    op.create_index(
        "idx_human_notes_translation_created_at",
        "human_notes",
...
def downgrade() -> None:
    ...
    # Drop composite indexes for WorkflowTask table removed
    # WorkflowTask tracking now handled by FastAPI app.state for in-memory storage
