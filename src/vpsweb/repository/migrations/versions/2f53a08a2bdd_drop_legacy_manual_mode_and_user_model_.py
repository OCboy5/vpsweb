...
def downgrade() -> None:
    ...
    with op.batch_alter_table('translation_workflow_steps', schema=None) as batch_op:
        ...
        batch_op.add_column(sa.Column('manual_mode', sa.BOOLEAN(), server_default=sa.text('(FALSE)'), nullable=True))
        batch_op.create_index(batch_op.f('idx_workflow_steps_manual_mode'), ['manual_mode'], unique=False)
