...
down_revision: Union[str, Sequence[str], None] = ('84e779a661b5', '90eb279fd688', 'add_background_briefing_report_table')
...
def upgrade() -> None:
    ...
    if not _index_exists(conn, "idx_translations_composite_created"):
        op.create_index("idx_translations_composite_created", "translations", ["poem_id", "created_at"])
    ...
    if not _column_exists(conn, "translations", "poet_subdirectory"):
        op.add_column("translations", sa.Column("poet_subdirectory", sa.String(length=100), nullable=True))
    if not _column_exists(conn, "translations", "relative_json_path"):
        op.add_column("translations", sa.Column("relative_json_path", sa.String(length=500), nullable=True))
    if not _column_exists(conn, "translations", "file_category"):
        ...
        op.add_column("translations", sa.Column("file_category", file_category_enum, nullable=True))

    # Add indexes for new fields
    if not _index_exists(conn, "idx_translations_poet_subdir"):
        op.create_index("idx_translations_poet_subdir", "translations", ["poet_subdirectory"], unique=False)
    if not _index_exists(conn, "idx_translations_file_category"):
        op.create_index("idx_translations_file_category", "translations", ["file_category"], unique=False)
    if not _index_exists(conn, "idx_translations_composite_file"):
        op.create_index("idx_translations_composite_file", "translations", ["poet_subdirectory", "file_category"], unique=False)
    ...
    if not _table_exists(conn, "translation_workflow_steps"):
        ...
        op.create_table(
            ...
            sa.Column("manual_mode", sa.Boolean(), nullable=False, server_default="false"),
        ...
        op.create_index("idx_workflow_steps_duration", "translation_workflow_steps", ["duration_seconds"])
        op.create_index("idx_workflow_steps_tokens", "translation_workflow_steps", ["tokens_used"])
        op.create_index("idx_workflow_steps_step_metrics", "translation_workflow_steps", ["step_type", "cost", "duration_seconds"])
        op.create_index("idx_workflow_steps_manual_mode", "translation_workflow_steps", ["manual_mode"])
    ...
    if not _table_exists(conn, "background_briefing_reports"):
        ...
        op.create_table(
            ...
            sa.Column("id", sa.String(length=26), nullable=False),
            sa.Column("poem_id", sa.String(length=26), nullable=False, unique=True, index=True),
            sa.Column("content", sa.Text(), nullable=False),
            sa.Column("model_info", sa.Text(), nullable=True),
            sa.Column("tokens_used", sa.Integer(), nullable=True, index=True),
            sa.Column("cost", sa.Float(), nullable=True, index=True),
...
def downgrade() -> None:
    ...
    op.drop_index("idx_workflow_steps_manual_mode", table_name="translation_workflow_steps")
    op.drop_index("idx_workflow_steps_step_metrics", table_name="translation_workflow_steps")
    ...
    op.drop_index("idx_workflow_steps_type_order", table_name="translation_workflow_steps")
    op.drop_index("idx_workflow_steps_workflow_id", table_name="translation_workflow_steps")
    op.drop_index("idx_workflow_steps_ai_log_id", table_name="translation_workflow_steps")
    op.drop_index("idx_workflow_steps_translation_id", table_name="translation_workflow_steps")
